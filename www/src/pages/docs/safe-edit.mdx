...
If exact text or replacement content contains shell metacharacters such as
backticks, prefer single quotes around exact text and `--content-file` for the
replacement body. For large Markdown or full-block replacements,
`--content-file` is usually safer than inline `--content`.
