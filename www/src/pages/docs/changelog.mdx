...
- Resolved TypeScript ESM runtime import specifiers such as `./module.js` to
  matching `.ts`, `.tsx`, `.mts`, and `.cts` source files.
- Resolved local Vite-style resource imports with query or hash suffixes such
  as `styles.css?url`.
