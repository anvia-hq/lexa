...
For MCP startup issues, add `--log-file <path>` so Lexa appends diagnostics
to a file you can inspect or attach to an issue:
...
```json
{
  "mcpServers": {
    "lexa": {
      "command": "lexa",
      "args": ["mcp", "/path/to/project", "--log-file", "/tmp/lexa-mcp.log"]
    }
  }
}
```
