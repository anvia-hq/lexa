...
The same tool also accepts exact text targets:
...
For anchor insertion, pass `anchor`, `placement: "before" | "after"`, and
`content`. Exact text and anchor modes must match one unique location. Patch
results include `line_count`, `lines_added`, `lines_removed`, and `preview`
for dry runs.
